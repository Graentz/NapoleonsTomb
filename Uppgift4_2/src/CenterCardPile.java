...
public class CenterCardPile implements CardPile{

    private Stack<Card> pile;
    private int x,y;
    private boolean isHighlighted;

    public CenterCardPile(int x, int y){
    ...
    public boolean add(Card card) {
        if (card != null){
            if (this.pile.empty() && card.getValue() == 6){
                this.pile.add(card);
                return true;
            }
            if (!this.pile.empty() && this.pile.peek().getValue() -1 == card.getValue()){
                this.pile.add(card);
                return true;
            }
            if(!this.pile.empty() && this.pile.peek().getValue() == 1 && card.getValue() == 6){
                this.pile.add(card);
                return true;
            }
        }
